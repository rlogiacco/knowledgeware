...
import org.agileware.kb.config.KafkaProperties;
...
class KnowledgewareKafkaResourceIT {
    ...
    void setup() {
        ...
        KnowledgewareKafkaResource kafkaResource = new KnowledgewareKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/knowledgeware-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/knowledgeware-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
